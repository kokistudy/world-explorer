...
def _draw_network(run, p_pass: float, bars: str = "initial") -> bytes:
    spec = run.spec
    positions = circle_layout(spec.agent_names)
    states = _thresholds_by_day(run)
    thresh_initial, _ = states[0]
    thresh_final, signed_by = states[-1]
    if bars == "updated":
        thresholds = thresh_final
        bar_note = "Bars: updated propensity"
    else:
        thresholds = thresh_initial
        bar_note = "Bars: initial propensity"
...
def _table_png(rows: list[dict], columns: list[tuple[str, str]]) -> bytes:
    ...
    for (r, c), cell_obj in table.get_celld().items():
        ...
        cell_obj.PAD = 0.08
        cell_obj.get_text().set_color("#1c2733")
        if r == 0:
...
with st.sidebar:
    ...
    st.number_input(
        "Signatures needed", min_value=1, max_value=20, key="need",
    )
    st.number_input(
        "Number of days", min_value=1, max_value=10, key="n_days",
    )
    n_samples = st.slider("Simulations", 200, 4000, 800, step=200)
    seed = st.number_input("Seed", min_value=0, value=1)
    st.subheader("Scores")
    st.caption("The first three match the explorer default. Turn on any extra scores.")
    selected = []
    for key, label, _header, default in SCORE_CHOICES:
        if f"score_{key}" not in st.session_state:
            st.session_state[f"score_{key}"] = default
        if st.checkbox(label, key=f"score_{key}"):
            selected.append(key)
...
with left:
    ...
    names_now = [
        str(r["name"]).strip()
        for _, r in agents_edited.dropna(subset=["name"]).iterrows()
        if str(r["name"]).strip()
    ]
    have = (
    ...
    if run_btn:
        try:
            ...
            if not names:
                ...
                schedule = _schedule(signed_edited, names, spec.n_days)
                ...
                columns = [
                    ("Person", "Person"),
                    ("Initial propensity", "Initial propensity"),
                ...
                for name in names:
                    ...
                    for key, _label in columns[3:]:
                        row[key] = f"{scores[name][key]:.3f}"
                    rows.append(row)
                display_rows = [
                    {label: row[key] for key, label in columns} for row in rows
                ]
                stem = f"toy_need{spec.pass_threshold}_{spec.n_days}d"
                st.session_state.sim_run = run
                st.session_state.p_pass = p_pass
                st.session_state.table_png = _table_png(rows, columns)
                st.session_state.scores_df = pd.DataFrame(display_rows)
                st.session_state.structure_name = f"{stem}_structure.png"
                st.session_state.table_name = f"{stem}_table.png"
        except ValueError as err:
            st.error(str(err))
    if st.session_state.get("sim_run") is not None and st.session_state.get("table_png"):
        st.subheader("Structure")
        bar_choice = st.radio(
            "Propensity bars",
            ["Initial", "Updated", "Both"],
            index=0,
            horizontal=True,
            key="bar_mode",
            help="Initial = starting bars. Updated = after influence from people who signed.",
        )
        run = st.session_state.sim_run
        p_pass = st.session_state.p_pass
        show_initial = bar_choice in ("Initial", "Both")
        show_updated = bar_choice in ("Updated", "Both")
        if show_initial:
            png_i = _draw_network(run, p_pass, "initial")
            st.image(png_i, use_container_width=True)
            st.download_button(
                "Download structure (initial)",
                data=png_i,
                file_name=st.session_state.structure_name.replace(
                    ".png", "_initial.png"
                ),
                mime="image/png",
                key="dl_structure_initial",
            )
        if show_updated:
            png_u = _draw_network(run, p_pass, "updated")
            st.image(png_u, use_container_width=True)
            st.download_button(
                "Download structure (updated)",
                data=png_u,
                file_name=st.session_state.structure_name.replace(
                    ".png", "_updated.png"
                ),
                mime="image/png",
                key="dl_structure_updated",
            )
        st.subheader("Table")
        if st.session_state.get("scores_df") is not None:
            st.dataframe(
                st.session_state.scores_df, use_container_width=True, hide_index=True,
            )
